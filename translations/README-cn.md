...
|^|从开始行开始匹配.|
|$|从末端开始匹配.|
|\b|单词边界符表示一个字符(\w)和一个非字符(\W)之间的位置|
|\B|\b的反向|
...
### 2.5.1 回溯引用(backreference)
分组另外一个衍生用法是实现回溯引用，从而保持前后一致效果。通过一对小括弧`(....)`标识一个分组，且从1开始从左到右记录下标，然后通过反斜杠`\`+数字下标识别组别
```
<([hH]\d)>.*?<\/\1>
```
示例中`\1`表示组别1，即([hH]\d)
regex101示例中最后一行前后标签不一致，所以未命中

[在线练习](https://regex101.com/r/aYp8uq/1)
...
先后相对位置说法是以断言表达式为中心，匹配表达式在左表示`先`;匹配表达式在其右侧表示`后`

`先`:表示匹配的表达式(`exp1`)在断言表达式(`exp2`)前面
`后`:表示匹配的表达式`exp1`在断言表达式(`exp2`)后面

### 4.1 `?=...` 正先行断言
语法：`exp1(?=exp2)`

正先行断言:表示寻找匹配`exp2`表达式,且其前面内容符合**`exp1`表达式的内容** 

返回结果只包含满足匹配条件的第一部分表达式(exp1).
...
负先行断言：寻找匹配`exp1`且后面不跟随`exp2`的表达式.与正先行断言（`?=`）相反
...
正后发断言: 用于筛选所有匹配`exp1`, 且前面匹配断言中exp2表达式的内容.
...
### 4.4 `?<!...` 负后发断言
语法：`(?<!exp2)exp1`
负后发断言 : 用于筛选所有匹配`exp1`结果,且前不跟随着断言中定义的exp2格式.
...
嵌入条件，表示如果那么意思。通过判断是否符合条件来判断后面的逻辑。嵌入条件主要两种：回溯引用和前后匹配查询
> 注:并不是所有语言都支持嵌入条件，JavaScript和java暂时不支持
...
```
false-regex非必选项
backReference 是分组下标(**这个分组不需要通过反斜杠，只需要分组下坐标即可**)，如果存在分组情况，则执行true-regex的正则表达式,反之执行false-regex表达内容

练习题：
(123)456-7890 123-456-7890都是可接受 北美电话号码格式
1234567890 (123)-456-7890 (123-456-7890) 虽然都包含着数目正
数字字符，但格式都不对，如何筛选正确格式电话？**回溯嵌入条件只是其中一个解答方式**
